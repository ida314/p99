"""The solutions screen: every way you know to solve every problem.

The same list the prompt after a solve edits, months later and out of a run. A
problem you have solved three ways has three rows here — the technique, what it
...
monotonic stack solution to this, it is the optimal one, and I have never
...
Two keys write:
  ...
  o  cycle what this way costs — optimal, not optimal, not sure, unclaimed
  e  open `$EDITOR` on the first way with no code, and archive what you write

Both go through the event log like everything else. `o` emits
`solution_updated`, which matters more than it looks: an optimal way that is not
the one you wrote is what `srs.rate` reads as "you knew there was better", so
marking one here feeds the scheduler for every future attempt at that problem.
`e` emits `solution_archived`, an event with no attempt on it — you did not sit
a timed problem, you sat down with a route you named months ago and finally
wrote it. It scores nothing for exactly that reason.

Deliberately not a schedule. Nothing here is due, and recording a second way
...
from ...render import solution_row
from ..vim import MOTIONS, VimMotion
from .solutions import OPTIMALITY_CYCLE


class SolutionsScreen(VimMotion, Screen[None]):
    """Problems on the left, that problem's ways on the right.
    ...
    BINDINGS = [
        *MOTIONS,
        ...
        Binding("l", "focus_ways", "ways", show=False),
    ]

    VIM_TARGET = "#solution-problems"

    def __init__(self) -> None:
        super().__init__()
        self.problems: list = []
        #: The ways of whichever problem the left cursor is on, in draw order,
        #: so `o` and `e` act without asking the database for a list they just
        #: rendered.
        self.ways: list[strategies.Solution] = []
        self._busy = False

    def compose(self) -> ComposeResult:
        yield Static("  solutions", classes="section-title")
        with Horizontal(id="solution-body"):
            yield OptionList(id="solution-problems")
            with VerticalScroll(id="solution-detail"):
                yield OptionList(id="solution-ways")
        yield Footer()
    ...
    def load(self) -> None:
        ...
        widget = self.query_one("#solution-problems", OptionList)
        highlighted = widget.highlighted
        self.problems = strategies.problems_with_solutions(conn)
        widget.clear_options()
        widget.border_title = f"{len(self.problems)} problems"
        if not self.problems:
            self.ways = []
            listing = self.query_one("#solution-ways", OptionList)
    ...
    def _problem_label(row) -> Text:
        """One problem: its title, how many ways, and how many have code.

        `2/3` rather than a bare count, because the ratio is what you scan the
        list for — three ways with one written is a different state from three
        of three, and it is the one worth walking over to.
        """
        line = Text("  ")
    ...
    def on_option_list_option_highlighted(self, event: OptionList.OptionHighlighted) -> None:
        if event.option_list.id == "solution-problems" and event.option.id:
            self._show(event.option.id)

    def _show(self, slug: str, focus_key: str | None = None) -> None:
        conn = self.app.conn  # type: ignore[attr-defined]
        self.ways = strategies.solutions(conn, slug)
        widget = self.query_one("#solution-ways", OptionList)
        highlighted = widget.highlighted
        widget.clear_options()
        ...
        if not self.ways:
            widget.add_option(
                Option(
                    Text("  no ways recorded for this problem yet", style="bright_black"),
                    id=None,
                    disabled=True,
                )
            )
    ...
    def _highlighted_way(self) -> strategies.Solution | None:
        widget = self.query_one("#solution-ways", OptionList)
        if widget.highlighted is None or not widget.option_count:
            return None
    ...
    def action_cycle_optimality(self) -> None:
        """Cycle the highlighted way through the four states, as an event.

        An event rather than an UPDATE because it feeds a rating: an optimal way
        that is not the one you wrote is what `srs.rate` reads as "you knew there
        was better", and anything a rating depends on has to survive a replay.
        """
        ...
        events.append(
            ...
            {"slug": slug, "solutions": [{"name": way.name, "optimality": nxt}]},
        )
    ...
    async def _do_write_code(self) -> None:
        """Open `$EDITOR` for the highlighted way, or the first one with no code.
        ...
        if way is None:
            self.notify("every way on this problem already has code")
            return
            ...
            try:
                with self.app.editor_context():  # type: ignore[attr-defined]
                    result = capture.capture_library_solution(
                        problem,
                        strategies.Strategy(key=way.key, name=way.name),
    ...
    def action_focus_problems(self) -> None:
        self.query_one("#solution-problems", OptionList).focus()

    def action_focus_ways(self) -> None:
        self.query_one("#solution-ways", OptionList).focus()
